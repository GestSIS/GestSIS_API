    ...
    <table class="table table-sm table-striped table-bordered mb-2 mt-3">
      <tr>
        ...
        <td><strong>Début</strong> : {{ str_replace('-', '.', $intervention->date_debut) }}
          {{ substr($intervention->heure_debut, 0, 5) }}
        </td>
        <td><strong>Fin</strong> : {{ str_replace('-', '.', $intervention->date_fin) }}
          {{ substr($intervention->heure_fin, 0, 5) }}
        </td>
      </tr>
      <tr>
    ...
    <table class="table table-sm table-bordered table-striped mb-2">
      ...
      <tr>
        ...
        <td>
          <div class="form-check text-center">
            <label class="form-check-label" for="remplace-{{ $sapeur['id'] }}"><input type="checkbox" class="form-check-input" id="remplace-{{ $sapeur['id'] }}" @if (array_key_exists($sapeur['id'], $quittances)) checked="checked" @endif>&#8203;</label>
          </div>
        </td>
      ...
      <tr>
        ...
        <td>{{ \Carbon\Carbon::parse($presence['debut'])->format('d.m H:i') }} -
          {{ \Carbon\Carbon::parse($presence['fin'])->format('H:i') }}
        </td>
